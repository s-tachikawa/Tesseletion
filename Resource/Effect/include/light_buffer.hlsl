...
float4 CalcVpl( float3 objPos, float3 objNormal, float3 litPos, float3 litNormal, float4 litColor, float4 litAttn )
{
	float3	eyeVec = -normalize( objPos );
	...
	float	litLen = length( litDir );
	float	litRate = litLen / litAttn.x;
	float	attn = max(1.0 - litRate, 0.0);
	if( attn == 0.0 )
	{
		return float4(0, 0, 0, 0);
	}
	...
	// ライト計算
	float	nl = saturate( dot( objNormal, litDir ) ) * dn;
	float3	halfVec = normalize( eyeVec + litDir );
	float	nh = saturate( dot( objNormal, halfVec ) );
	float	power = litAttn.w;
	float	spec = 0.0;
...
float4 CalcVplBRDF( float3 objPos, float3 objNormal, float3 litPos, float3 litNormal, float4 litColor, float4 litAttn, float gloss, float f0 )
{
	float3	eyeVec = -normalize( objPos );
