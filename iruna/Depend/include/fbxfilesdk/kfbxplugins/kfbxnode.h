    ...
    class KFBX_DLL KFbxNode : public KFbxObject
    {
        ...
        public:
			...
			/**
              ...
              */
            //@{
                /** Invalidate the internal caches to force evaluation on calls to SetLocalStateFrom...
                  * \param pGlobal  If set to \c true, invalidate the caches for the global state.
                  */
                void ForceEvalOfState(bool pGlobal=false);

                /** Load in local state the translation, rotation and scaling values relative to 
				  * parent at a given time.
                ...
                K_DEPRECATED void SetLocalStateFromDefaultTake(bool pRecursive, bool pApplyLimits = false);

                /** Store local state as translation, rotation and scaling vectors relative to 
				  * parent at a given time.
                  * \param pRecursive Flag to call the function recursively to children nodes.
                  * \remarks Translation, rotation and scaling vectors relative to parent are written
				  *          in default take.
                  */
                void SetDefaultFromLocalState(bool pRecursive);
                K_DEPRECATED void SetDefaultTakeFromLocalState(bool pRecursive);

                /** Load in local state the translation, rotation and scaling values relative to 
				  * parent at a given time.
                  * \param pTime Given time to evaluate transform vectors.
                  * \param pRecursive Flag to call the function recursively to children nodes.
                  * \param pAnimStack  Animation stack 
                  * \param pApplyLimits true if node limits are to be applied on result
                  * \remarks TRS position relative to parent is read from current take.
                  * \remarks This method will only inspect the first animation layer connected 
                  *          to the specified animation stack.
                  */
                void SetLocalStateFromAnim(KTime pTime, bool pRecursive, KFbxAnimStack* pAnimStack, bool pApplyLimits = false);
                K_DEPRECATED void SetLocalStateFromCurrentTake(KTime pTime, bool pRecursive,bool pApplyLimits = false);

                /** Store local state as translation, rotation and scaling vectors relative to 
				  * parent at a given time.
                  * \param pTime Given time to store the transform vectors.
                  * \param pRecursive Flag to call the function recursively to children nodes.
                  * \param pAnimStack  Animation stack 
                ...
                K_DEPRECATED void SetCurrentTakeFromLocalState(KTime pTime, bool pRecursive);

                /** Compute global state from local state.
                  * \param pUpdateId Update ID to avoid useless recomputing.
                  * \param pRecursive Flag to call the function recursively to children nodes.
				  * \param pPivotSet Pivot context.
                  * \param pApplyTarget Applies the necessary transform to align into the target node
                ...
                /** Set global state.
                  * \param pGX Global transform matrix.
                  */
                ...
                /** Set local state.
                  * \param pLT position relative to parent.
				  * \param pLR rotation relative to parent.
				  * \param pLS scaling relative to parent.
                  */
